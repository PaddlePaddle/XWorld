...
const int STADIUM_SCALE = 0.01f;

X3Scene::X3Scene(float gravity, float time_step, int frame_skip, bool big_screen):
...
void X3Scene::build_scene(const std::vector<X3ItemInfo>& infos,
    ...
    Pose stadium_pose(0.0f, 0.0f, 0.0f);
    floor_ = world_.load_thingy(stadium_file,
                                stadium_pose,
                                STADIUM_SCALE,
                                0.0f, // mass: 0 means that the stadium is non-movable
                                0xffffff,
                                true);
...
void X3Scene::add_item(const X3ItemInfo& info) {
    if (info.type == AGENT) {
        ...
        auto agent = std::make_shared<X3Agent>(info, world_,
                                               FLAGS_x3_move_speed * UNIT,
                                               FLAGS_x3_jump_speed * UNIT,
                                               FLAGS_x3_orientation_bins,
                                               FLAGS_x3_reaching_distance * UNIT);
...
X3Goal* X3Scene::collect_goal(const X3AgentPtr& agent) {
    ...
    // the direction from the agent to the goal should be less than 45 degrees
    float best_score = 0.707; // 45 degrees is the minimum
    float score;
    for (auto& it : goals_) {
