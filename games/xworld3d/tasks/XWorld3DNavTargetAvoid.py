...
This task asks the agent not to go to a certain object.

Example:
Please avoid the apple.
"""
...
class XWorld3DNavTargetAvoid(XWorld3DTask):
    ...
    def idle(self):
        ...
        referents = [g for g in goals if g.name != sel_goal.name]
        assert referents, "Identical object names?"
        referent = random.choice(referents)

        self._record_target([g for g in goals if g.name != referent.name])
        self._bind("S -> start")
