...
std::string XWorldLanDirectionToObjectTask::idle(ScannerPtr scanner,
                                                 SentenceTemplatePtr sen_temp,
                                                 TeachingEnvPtr game) {
...
void XWorldLanDirectionToObjectTask::define_sen_temp_rules(
    SentenceTemplatePtr sen_temp, TeachingEnvPtr game) {
    // will increase the variations of the questions later
    sen_temp->add_rule(sen_temp->start_symbol(), {"what is on the $D"});
    sen_temp->add_rule("$D", directions_, true /*must_bound*/);
}

void XWorldLanDirectionToObjectTask::define_sen_temp_rules_for_answer(
    SentenceTemplatePtr sen_temp) {
...
std::vector<std::vector<Entity>> XWorldLanDirectionToObjectTask::find_goal(
    ScannerPtr scanner) {
...
void XWorldLanDirectionToObjectTask::generate_sentence(
    ...
    sen_temp_answer->add_rule(
        "$O", {goal_set[0].property("name")}, true /*must bound*/);
