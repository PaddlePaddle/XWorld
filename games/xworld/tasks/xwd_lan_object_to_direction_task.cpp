...
void XWorldLanObjectToDirectionTask::define_sen_temp_rules(SentenceTemplatePtr sen_temp,
                                                           TeachingEnvPtr game) {
...
void XWorldLanObjectToDirectionTask::define_sen_temp_rules_for_answer(SentenceTemplatePtr sen_temp) {
    sen_temp->add_rule(sen_temp->start_symbol(), {
                     "on the $D is $O",
                    "$O is on the $D"});
...
std::vector<std::vector<Entity>> XWorldLanObjectToDirectionTask::find_goal(ScannerPtr scanner) {
...
void XWorldLanObjectToDirectionTask::generate_sentence(
    ...
    sen_temp->add_rule("$O", {goal_set[0].property("name")}, true/*must_bound*/);
    ...
    sen_temp_answer->add_rule("$O", {goal_set[0].property("name")}, true/*must_bound*/);
