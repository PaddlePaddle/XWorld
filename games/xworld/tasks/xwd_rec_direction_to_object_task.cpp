...
std::string XWorldRecDirectionToObjectTask::idle(ScannerPtr scanner,
                                                 SentenceTemplatePtr sen_temp,
                                                 TeachingEnvPtr game) {
...
void XWorldRecDirectionToObjectTask::define_sen_temp_rules(
...
std::vector<std::vector<Entity>> XWorldRecDirectionToObjectTask::find_goal(
    ScannerPtr scanner) {
...
void XWorldRecDirectionToObjectTask::generate_sentence(
    ...
    sen_temp->bind(
        "$G", get_direction(scanner->agent_.location, goal_set[0].location));
