...
void XMaze::modify_blocks(
    ...
    for (int i = blocked.size(); i < n_blocks; i++) {
        ...
        while (true) {
            ...
            if (defined_reachable_locs.find(empty_lot) ==
                defined_reachable_locs.end())
                break;
        }
...
int XMaze::dfs(int id,
    ...
    for (int i = 0; i < 4; i++) {
        yy = y + dy[i];
        xx = x + dx[i];
        if (yy >= 0 && xx >= 0 && yy < height_ && xx < width_ &&
            visited[yy][xx] == -1 && maze_[yy][xx]) {
...
void XMaze::bfs(int y,
    ...
    while (!que.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            jj = j + dx[k];
            next = merge(ii, jj);
            if (ii >= 0 && ii < height_ && jj >= 0 && jj < width_ &&
                !visited[next]) {
                visited[next] = true;
                que.push(next);
                prev[next] = cnt;
...
void XMaze::get_locations(std::vector<Loc>& locations, int type) const {
    ...
    switch (type) {
        case LARGEST_COMP:
            qualifier = [this](int i, int j) { return largest_comp_[i][j]; };
            break;
        case EMPTY:
            qualifier = [this](int i, int j) { return maze_[i][j]; };
            break;
        case BLOCK:
            qualifier = [this](int i, int j) { return !maze_[i][j]; };
            break;
    ...
    for (int i = 0; i < height_; i++) {
        for (int j = 0; j < width_; j++) {
            if (qualifier(i, j)) locations.push_back({j, i});
        }
    }
}
