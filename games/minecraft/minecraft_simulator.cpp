...
MinecraftSimulatorBase::MinecraftSimulatorBase(const std::string& conf_path) {
    ...
    client_pool_.add(malmo::ClientInfo(FLAGS_minecraft_client_ip,
                                       FLAGS_minecraft_client_port));
    // action_names_ = {"movenorth 1", "movesouth 1", "movewest 1", "moveeast
    // 1"};
...
void MinecraftSimulatorBase::reset_game() {
    ...
    for (int retry = 0; retry < max_retries; ++retry) {
        try {
            agent_host_.startMission(*mission_,
                                     client_pool_,
                                     mission_record,
                                     /* role= */ 0,
...
float MinecraftSimulatorBase::observe() {
    ...
    auto obs = world_state_.observations.empty()
                   ? nullptr
    ...
    while (world_state_.is_mission_running) {
        ...
        if (reached && !world_state_.observations.empty() &&
            world_state_.observations.back()->text != "{}" &&
...
void MinecraftSimulatorBase::get_screen(StatePacket& screen) {
    ...
    std::copy(frame.pixels.begin(), frame.pixels.end(), img.ptr<uchar>());
    ...
    for (int h = 0; h < height; ++h) {
        for (int w = 0; w < width; ++w) {
            for (size_t c = 0; c < 3; c++) {
                ...
                screen_vec[offset] = img.at<cv::Vec3b>(cv::Point(w, h)).val[c];
            }
        }
    }
#endif
...
void MinecraftSimulatorBase::show_screen(float rewards) {
    auto& frame = *world_state_.video_frames.back();
    cv::Mat img(
        cv::Size(frame.height, frame.width), CV_8UC3, frame.pixels.data());
