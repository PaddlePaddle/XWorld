...
namespace xrobot {
	...
	Lidar::Lidar(World* world, const int number_of_rays, const float max_distance) :
            world_(world),
	...
	void Lidar::Update(
		...
		for (int i = 0; i < num_rays_; ++i) {
	        glm::quat lidar_orentation =
                    glm::angleAxis(horizontal_segment_angle_ * i, up);
	        glm::vec3 lidar_ray_direction =
                    glm::rotate(lidar_orentation, front);
	        glm::vec3 lidar_far_position =
                    lidar_ray_direction * max_distance_ + center;
