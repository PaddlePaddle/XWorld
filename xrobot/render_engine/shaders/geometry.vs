...
void main()
{
    ...
    vs_out.Pos = vec3(matrices.state * matrices.model * matrices.scale * vec4(aPos, 1.0));
    vs_out.Normal = normalize(n * aNormal);
    vs_out.Tangent = normalize(n * aTangent);
    vs_out.BiTangent = normalize(n * aBiTangent);
    vs_out.ViewPos = vec3(view * vec4(vs_out.Pos, 1.0));
    vs_out.ViewNormal = normalize(mat3(view) * vs_out.Normal);
