...
GLFWContext::GLFWContext(int h, int w, bool core): GLContext{h, w}, window_(nullptr) {
  glfwInit();
...
GLXVisualizationContext::GLXVisualizationContext(int h, int w) : GLContext{h, w} {
  ...
  int numberOfFramebufferConfigurations = 0;
  GLXFBConfig* fbc = glXChooseFBConfig(dpy_,
    scnId_, glxAttribs, &numberOfFramebufferConfigurations);
  ...
  if (scnId_ != visual->screen) {
    std::cout << "screenId(" << scnId_ << ") does not match visual->screen(" << visual->screen << ").\n";
    XCloseDisplay(dpy_);
    exit(1);
  }
  ...
  windowAttribs.colormap = XCreateColormap(dpy_, RootWindow(dpy_, scnId_), visual->visual, AllocNone);
  windowAttribs.event_mask = ExposureMask;
  win_ = XCreateWindow(dpy_, RootWindow(dpy_, scnId_), 0, 0, w, h, 0, visual->depth, InputOutput, visual->visual, CWBackPixel | CWColormap | CWBorderPixel | CWEventMask, &windowAttribs);
