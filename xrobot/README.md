...
Some examples can be found in `/python`. Below we show step-by-step code to create a navigation task.
...
def __init__(self):
	...
	## YOU SHOULD CREATE THIS PLAYGROUND OUTSIDE OF TASK. THIS IS ONLY FOR DEMOSTRATION.
	...
	## the rendering quality and assign it to a dedicate gpu (if you have a multi-gpu
	## system)

	## Headless Context
	## Headless context is used for rendering a scene without assosicating a window
	...
	## (This only available when rendering is debug mode)

	## Rendering Quality
	## Rendering Quality can be choosed between low and normal. Low quality does not
	## have any of post-processing but is quite fast. However, normal quality use SSAO  
	## to increase some small occulsion-like details and use FXAA to reduce some of 
	...
	## Select GPU (Only available in multi-gpu system)
	##
	## Use gpu id to assign the rendering backend to a dedicate gpu
	## (gpu id can be find in nvidia-smi)
...
def start(self):
	...
	## SpawnAnObject
	## This function can use to generate a object with a certain position, orientation and
	## uniform scale. Object label has to be assigned for query purposes. Last but not least,
	## the prpperty of fixed root also need to be assigned. This defines whether or not the
	## object will be treated as an active ridigbody in simulation. 
	##
	## Be award of orientation representation! It uses valid quaternion for orientation!
...
class XRobotEnv(object):
    def __init__(self):
        ...
        ## Create a task group. A task group is a container for same type of tasks.
        ## By calling the function 'run_stage' to randomly select a task in task group.
...
Do not forget use `__GL_SYNC_TO_VBLANK=0` to disable V-Sync.
