...
FBO::FBO(
	GLuint w, GLuint h, bool depthBuffer, bool sigBuffer,
...
)
{
    ...
    if (sigBuffer) {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrap);
        glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, w, h, 0, GL_RGBA, format, NULL);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, textureSigBuffer, 0);
...
FBO::~FBO()
{
	...
	if(needDepthBuffer == false) glDeleteRenderbuffers(1, &rbo);
	else glDeleteTextures(1, &textureDepthBuffer);
    if(needSigBuffer) glDeleteTextures(1, &textureSigBuffer);
}

void FBO::ActivateSgAsTexture(const int shaderProgram, const std::string glSamplerName, const int textureUnit)
{
...
void FBO::ActivateDepthAsTexture(const int shaderProgram, const std::string glSamplerName, const int textureUnit)
{
